...
class T5MemTokenizerFast(PreTrainedTokenizerFast):
    auto_tokenizer_loader = AutoTokenizer
    auto_config_loader = AutoConfig
    slow_tokenizer_class = T5MemTokenizer

    def __init__(self, *args, **kwargs):
        super(T5MemTokenizerFast, self).__init__(*args, **kwargs)

    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        assert isinstance(pretrained_model_name_or_path, str), (
            "pretrained_model_name_or_path must be a path to a "
        ...
        if os.path.exists(pretrained_model_name_or_path):
            ...
            if pretrained_model_name_or_path.endswith("json"):
                ...
                return cls.auto_tokenizer_loader.from_pretrained(
                    config.underlying_config, *init_inputs, **kwargs
                )
            else:
                # otherwise, it is a config json path
                raise NotImplementedError(
                    "loading a pretrained saved T5Mem checkpoint is not yet implemented"
                )
        else:
