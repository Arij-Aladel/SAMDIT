...
def _make_mem_seq_relative_position_ids(num_slots: int, mem_size: int, block_size:int) -> torch.Tensor:  # modified from original longt5
    ...
    seq_side_ids = torch.arange(block_size) + 1 # position of segments tokens from the related memory
    seq_side_ids = seq_side_ids.repeat((mem_realtive_positions.shape[1],1))
    mem_total_relative_position = torch.cat([mem_realtive_positions,seq_side_ids], dim=1)#global_positions - block_ids[..., None]
...
# TODO
def _make_side_relative_position_ids(attention_mask: torch.Tensor, global_block_size: int, num_mem: int) -> torch.Tensor: # oroginal longt5
    """Create the relative position tensor for local -> global attention."""
    block_ids, global_segment_ids = _make_global_fixed_block_ids(attention_mask, global_block_size, num_mem)
    block_ids, global_segment_ids = block_ids.to(attention_mask.device), global_segment_ids.to(attention_mask.device)
...
def _make_global_fixed_block_ids(attention_mask: torch.Tensor, block_size: int, num_mem: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    fixed_block_mask = torch.ones_like(attention_mask, device=attention_mask.device) / block_size
...
class T5MemAttention(nn.Module):
    def __init__(self, config: T5MemConfig, has_relative_attention_bias=False): # (self, config: T5MemConfig, num_blocks: int,  has_relative_attention_bias=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        ...
        if self.has_relative_attention_bias:  
            ...
            self.relative_attention_bias = nn.Embedding(self.relative_attention_num_buckets, self.n_heads)           # se_seq_embedding seq = rows= query, seq = columns = key used in  compute_bias function
            # seq_mem positional embedding
            self.seq_mem_relative_attention_bias = nn.Embedding(self.relative_attention_num_buckets, self.n_heads)   # seq=rows= query, mem column=key  used in compute_side_bias function
            # mem_total positional embedding; total =[mem;seq]
            self.mem_total_relative_attention_bias = nn.Embedding(self.relative_attention_num_buckets, self.n_heads) # me =rows = query, total = [mem:seq] = columns = keys used in  compute_mem_total_bias  function
    ...
    def compute_bias(self, block_size, num_blocks, device=None):  # used for seq_seq bias
        """Compute binned relative position bias"""
        context_position = torch.arange(block_size, dtype=torch.long, device=device)[:, None]
        memory_position = torch.arange(block_size, dtype=torch.long, device=device)[None, :]
        relative_position = memory_position - context_position  # shape (query_length, key_length)
        relative_position_bucket = self._relative_position_bucket(
            relative_position,  # shape (query_length, key_length)
            bidirectional=(not self.is_decoder),
        ...
        values = self.relative_attention_bias(relative_position_bucket)  # shape (query_length, key_length, num_heads)
        values = values.permute([2, 0, 1]).unsqueeze(0)## shape (1,num_heads, query_length, key_length)  #  (1, 1, num_heads, block_length, 3 * block_length) longt5
        values = values.unsqueeze(0).expand(-1, num_blocks, -1,-1,-1) # (1, num_block, num_heads, query_length, key_length) 
        return values
    #not clean yet : imperical  seq_mem_attention_side_bias
    def compute_side_bias(self, mask: torch.Tensor, block_ids:int , global_segment_ids: int) -> torch.Tensor:  # To compute the bias between memory slots and the seqments
        '''
        compute masked   seq_mem bias 
        '''
        # (batch_size, seq_len, global_seq_len)
        side_relative_position = _make_side_relative_position_ids(mask, self.block_size, self.num_mem)  
        ...
        attention_side_bias = torch.where(side_attention_mask > 0, 0.0, torch.finfo(torch.float).min)  # this is the mask
    ...
    def compute_mem_total_bias(self, mask: torch.Tensor, block_size:int,  mem_size:int,  num_slots:int) -> torch.Tensor: # in my model
        '''
        new verion
        ...
        side_relative_position = _make_mem_seq_relative_position_ids(num_slots, mem_size, block_size).unsqueeze(0)
        side_relative_position = einops.repeat(side_relative_position, 'b m n -> b k m  n', k=num_slots).to(mask.device)
        ...
        mem_seq_bias = self.mem_total_relative_attention_bias(mem_seq_relative_position_bucket)
        ...
        mem_seq_attention_side_bias = torch.where(mem_seq_attention_mask > 0, 0.0, torch.finfo(torch.float).min)  # this is the mask
        ...
        mem_seq_attention_side_bias = mem_seq_attention_side_bias[:,:,None,...] + mem_seq_bias
        # print("mem_seq_attention_side_bias min: ", mem_seq_attention_side_bias.min())

        # mem_seq_attention_side_bias = mem_seq_attention_side_bias.unsqueeze(2).expand(-1, -1, self.config.num_heads,-1,-1) 

        return mem_seq_attention_side_bias.to(mask.device), mem_seq_attention_mask[:,:,None,...].to(mask.device)
    ...
    def forward(
        ...
        def shape(states):
            ...
            return states.view(batch_size, self.num_slots, -1, self.n_heads, self.key_value_proj_dim)

        def unshape(states):
            """reshape"""
              #(batch_size, num_blocks, block_size, inner_dim)
            return states.contiguous().view(batch_size, self.num_slots, -1, self.inner_dim)
        ...
        scores = torch.einsum("...qhd,...khd->...hqk", query_states, key_states)  # equivalent of torch.einsum("bnqd,bnkd->bnqk", query_states, key_states), compatible with onnx op>9
        ...
        if position_bias is None:
            if not self.has_relative_attention_bias:
                ...
                position_bias = torch.zeros(
                    (1, self.n_heads, self.block_size+self.num_mem* self.num_slots, self.block_size+self.num_mem* self.num_slots), device=scores.device, dtype=scores.dtype
                )
                ...
                block_ids, global_segment_ids = _make_global_fixed_block_ids(mask, self.block_size, self.num_mem)
                block_ids, global_segment_ids = block_ids.to(hidden_states.device), global_segment_ids.to(hidden_states.device)
                #  (batch_size, num_heads, seq_len, global_seq_len)
                side_position_bias = self.compute_side_bias(mask, block_ids, global_segment_ids)  # masked
                side_position_bias = side_position_bias.type(scores.dtype).to(scores.device)
                # batch_size, 
                mem_total_position_bias, mem_total_attention_mask = self.compute_mem_total_bias(mask, self.block_size,  self.num_mem,  self.num_slots) 
            ...
            if mask is not None:
                ...
                splitted_mask = _split_into_blocks(mask.unsqueeze(2), self.block_size,-2)
                ...
                position_bias = position_bias + splitted_mask  # (batch_size, num_blocks,num_heads, block_size, block_size)
                ...
                side_position_bias = _split_into_blocks(side_position_bias, self.block_size, dim=-2).transpose(1, 2)


                seq_total_position_bias = torch.cat([side_position_bias, position_bias], dim=-1)
                ...
                position_bias = torch.cat([mem_total_position_bias, seq_total_position_bias], dim=-2)
        ...
        # present_key_value_state = (key_states, value_states) if (self.is_decoder and use_cache) else None
        outputs = (attn_output, position_bias,)  #  (attn_output, position_bias,) 


        if output_attentions:
            outputs = outputs + (attn_weights,mem_total_attention_mask,)  # (attn_output,position_bias, attn_weights, mem_seq_attention_mask)
...
class T5MemLayerAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.Attention = T5MemAttention(config, has_relative_attention_bias=has_relative_attention_bias)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
        self.config = config
...
class T5MemBlock(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.is_decoder = config.is_decoder
        self.layer = nn.ModuleList()
        self.layer.append(T5MemLayerAttention(config, has_relative_attention_bias=has_relative_attention_bias))
        self.layer.append(T5LayerFF(config))
...
class T5MemPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
        ...
        elif isinstance(module, (T5MemModel,T5MemEncoderModel, T5MemForConditionalGeneration)): # TODO T5MemForConditionalGeneration is added
            ...
            if module.has_relative_attention_bias:
                module.relative_attention_bias.weight.data.normal_(mean=0.0, std=factor * ((d_model) ** -0.5))
                module.seq_mem_relative_attention_bias.weight.data.normal_(mean=0.0, std=factor * ((d_model) ** -0.5))
                module.mem_total_relative_attention_bias.weight.data.normal_(mean=0.0, std=factor * ((d_model) ** -0.5))

    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (T5Attention, T5Stack, T5MemAttention, T5MemStack)): # TODO T5MemAttention, T5MemStack are added
...
class T5MemStack(T5MemPreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.block = nn.ModuleList(
            [T5MemBlock(config, has_relative_attention_bias=bool(i == 0)) for i in range(config.num_layers)]
        )
        self.final_layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
#         print("=================================================================================================\n\n")
        ...
        return EncoderOutput(
            last_hidden_state=hidden_states,
            last_memory_hidden_state=mem_upd[:,0,:,:].reshape(batch_size,-1,self.config.d_model),
            hidden_states=all_hidden_states,
            attentions=all_attentions,
        )
...
class T5MemModel(T5MemPreTrainedModel):
    ...
    def forward(
        ...
        elif return_dict and not isinstance(encoder_outputs, BaseModelOutput):  # maybe need to change
...
@add_start_docstrings("""T5Mem Model with a `language modeling` head on top.""", T5Mem_START_DOCSTRING)
class T5MemForConditionalGeneration(T5MemPreTrainedModel):
    ...
    # @add_start_docstrings_to_model_forward(T5_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=MemSeq2SeqLMOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        ...
        if encoder_outputs is None:
            ...
            # print("0:   encoder_outputs.last_hidden_state.shape shape >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  ", encoder_outputs.last_hidden_state.shape, encoder_outputs[0].shape)
            # print("-------encoder_outputs.last_hidden_state[0,0,:15]", encoder_outputs.last_hidden_state[0,0,:15])
            # print("-------encoder_outputs.last_hidden_state[1,0,:15]", encoder_outputs.last_hidden_state[1,0,:15])
            # print("-------encoder_outputs.last_hidden_state[0,:15,0]", encoder_outputs.last_hidden_state[0,:15,0])
            # print("-------encoder_outputs.last_hidden_state[1,:15,0]", encoder_outputs.last_hidden_state[1,:15,0])
            # print("0:   encoder_outputs.last_memory_hidden_state.shape  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", encoder_outputs.last_memory_hidden_state.shape)
            ...
            # print("1:   encoder_outputs.last_hidden_state.shape shape >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  ", encoder_outputs.last_hidden_state.shape)
            # print("1:   encoder_outputs.last_memory_hidden_state.shape  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", encoder_outputs.last_memory_hidden_state.shape)
            '''
            ...
            print("memory = encoder_outputs[1]---------------- ", memory.shape  )
            print("2:   encoder_outputs.last_hidden_state.shape shape   >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", encoder_outputs.last_hidden_state.shape)
            print("2:   encoder_outputs.last_memory_hidden_state.shape  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", encoder_outputs.last_memory_hidden_state.shape)
        ...
        attention_mask = _split_into_blocks(attention_mask, block_len = self.config.block_size, dim=1)
        # batch_size, num_blocks*block_size
        attention_mask = attention_mask.reshape((attention_mask.shape[0], hidden_states.shape[1]))
        ...
        if self.config.tie_word_embeddings:
            ...
            encoder_memory_hidden_states = encoder_outputs[1] * (self.model_dim**-0.5)  # Why we are doing that??
        ...
        encoder_logits = self.lm_head(torch.cat([encoder_memory_hidden_states,encoder_hidden_states[0:1,:,:]], dim=1))  # in case num_beam>1
        ...
        # fix 8_12_2022
        if not return_dict:
            output = (lm_logits,) + decoder_outputs[1:] + encoder_outputs + (encoder_logits[:,memory.shape[1]:,:], encoder_logits[:,:memory.shape[1],:])
            return ((loss,) + output) if loss is not None else output

#         print("encoder_logits[:,:memory.shape[1],:] shape:  ", encoder_logits[:,:memory.shape[1],:].shape)
#         print("encoder_logits[:,memory.shape[1]:,:].shape:  ", encoder_logits[:,memory.shape[1]:,:].shape)
        return MemSeq2SeqLMOutput(
            ...
            encoder_memory_logits=encoder_logits[:,:memory.shape[1],:]
        )
        ...
        **model_kwargs,
        ...
        if is_encoder_decoder:
            ...
            if "last_memory_hidden_state" in encoder_outputs:
                encoder_outputs["last_memory_hidden_state"] = encoder_outputs.last_memory_hidden_state.index_select(
                    0, expanded_return_idx.to(encoder_outputs.last_hidden_state.device)
                )
