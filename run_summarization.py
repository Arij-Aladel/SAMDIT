...
import models
from models import T5MemConfig
...
summarization_name_mapping = {
    "amazon_reviews_multi": ("review_body", "review_title"), # yes
    "big_patent": ("description", "abstract"),
    "cnn_dailymail": ("article", "highlights"),
    "orange_sum": ("text", "summary"),
    "pn_summary": ("article", "summary"),
    "psc": ("extract_text", "summary_text"),
    "samsum": ("dialogue", "summary"), #  does not work
    "thaisum": ("body", "summary"),# ThaiSum is a large-scale corpus for Thai text summarization obtained from several online news websites namely Thairath
    "xglue": ("news_body", "news_title"), # nc , ntg
    "xsum": ("document", "summary"),
    "wiki_summary": ("article", "highlights"),
    "ccdv/arxiv-summarization":("article","abstract"), # No
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The path to preprocessed data on the Disk).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max_length",
        type=int,
        default=128, # 173,#128*4,#128*3, #173
        help=(
    ...
    # Sanity checks
    if args.dataset_name is None and args.train_file is None and args.validation_file is None and args.dataset_name_cached is None :
        raise ValueError("Need either a dataset name or a training/validation file or dataset_name_cached.")
    else:
...
def main():
    ...
    if args.model_name_or_path: # for now here
        if args.model_name_or_path in ['configs/t5mem-small.json', 'configs/t5mem-base.json', 'configs/t5mem-large.json']:
            model = AutoModelForSeq2SeqLM.from_config(config)
            # initilize from pretrained t5
            t5_model = AutoModelForSeq2SeqLM.from_pretrained("t5-base")
            def load_state_dict(mymodel, model_dict, skip_wrong_shape: bool = False):

                mymodel_dict = mymodel.state_dict()
                for key in model_dict.keys():
                    if key not in mymodel_dict.keys():
                        my_key = key[:key.index('SelfAttention')]+key[key.index('SelfAttention')+len('Self'):]
                    else:
                        my_key = key

                    if mymodel_dict[my_key].shape == model_dict[key].shape:
                        mymodel_dict[my_key] = model_dict[key]
                    elif not skip_wrong_shape:
                        m = (f"Shapes of the mymodel_dict['{my_key}'] vs  model_dict[{key}]parameters do not match: "
                            f"{mymodel_dict[my_key].shape} vs {model_dict[key]}")
                        raise Exception(m)
    ...
    with accelerator.main_process_first():
        if not args.dataset_name_cached:
            ...
            def preprocess_len_function(examples):
                ...
                output["target_len"] = [len(x) for x in examples["labels"]]
                return output
            processed_datasets_len = processed_datasets.map(
                preprocess_len_function,
                batched=True,
                num_proc=args.preprocessing_num_workers,
    ...
    accelerator_log_kwargs["Num examples"] = len(train_dataset)
    accelerator_log_kwargs["total_batch_size"] = total_batch_size
...
#             accelerator.save_state(output_dir)
        ...
        for step, batch in enumerate(test_dataloader):
            with torch.no_grad():
                ...
                # We keep track of the loss at each epoch
                if args.with_tracking:
                    test_total_loss += test_loss.detach().float()
        ...
        result_test = metric.compute(use_stemmer=True)
        result_test = {k: round(v * 100, 4) for k, v in result_test.items()}
        ...
        if args.with_tracking:
            result["train_loss"] = total_loss.item() / len(train_dataloader)
            result["eval_loss"] = eval_total_loss.item() / len(eval_dataloader)
            result["test_loss"] = test_total_loss.item() / len(test_dataloader)
            result["epoch"] = epoch
            result["step"] = completed_steps
            result["test_rouge1"] = result_test["rouge1"]
            result["test_rouge2"] = result_test["rouge2"]
            result["test_rougeL"] = result_test["rougeL"]
            result["test_rougeLsum"] = result_test["rougeLsum"]
            result["nm_model_params"] = total_params
            accelerator.log(result, step=completed_steps)
            ...
            json.dump(
                {
                    ...
                    "test_rouge1": result_test["rouge1"],
                    "test_rouge2": result_test["rouge2"],
                    "test_rougeL": result_test["rougeL"],
                    "test_rougeLsum": result_test["rougeLsum"],
                    # "prefix_length":prefix_length.item() if prefix_length else 0 ,
                    "num_parametrs":total_params
                },
                f,
            )
