...
# accelerate launch --config_file='./accelerate.yaml' run_summarization_no_trainer_test.py  \
                #    --seed=42 \
                #    --preprocessing_num_workers=1 \
                #    --weight_decay='0.001' \
                #    --output_dir="arxiv-summarization_exp/t5mem/base" \
                #    --per_device_train_batch_size=1 \
                #    --per_device_eval_batch_size=1    \
                #    --num_train_epochs=10  \
                #    --model_name_or_path='t5mem-base' \
                #    --num_beams=5 \
                #    --with_tracking \
                #    --report_to='wandb' \
...
# export CUDA_LAUNCH_BLOCKING=1 WANDB_CONSOLE=off TORCH_DISTRIBUTED_DEBUG=INFO  
# accelerate launch --config_file='./accelerate.yaml' run_summarization_no_trainer_sled.py \
                    # --seed=42 \
                    # --preprocessing_num_workers=1 \
                    # --weight_decay='0.001' \
                    # --output_dir="cnn_dailymail_exp/SLED_256-10ep_1024_input_128TrainEvalOutput" \
                    # --per_device_train_batch_size=1 \
                    # --per_device_eval_batch_size=1    \
                    # --num_train_epochs=10  \
                    # --model_name_or_path='tau/t5-v1_1-base-sled' \
                    # --num_beams=5 --with_tracking \
                    # --report_to='wandb' \
                    # --checkpointing_steps='epoch' \
...
# # # ###  SAmSum
# export CUDA_LAUNCH_BLOCKING=1 WANDB_CONSOLE=off TORCH_DISTRIBUTED_DEBUG=INFO
# accelerate launch --config_file='./accelerate.yaml'  run_summarization.py  \
#                   --seed=42 \
#                   --preprocessing_num_workers=1 \
#                   --weight_decay='0.001'   \
#                   --output_dir="examples/samsum_exp2"   \
#                   --per_device_train_batch_size=1 \
#                   --per_device_eval_batch_size=1    \
#                   --num_train_epochs=5  \
#                   --model_name_or_path='configs/t5mem-base.json' \
#                   --num_beams=5  \
#                   --with_tracking  \
#                   --report_to='wandb' \
#                   --checkpointing_steps='epoch'  \
